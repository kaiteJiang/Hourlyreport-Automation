...
自动发现失败或安装位置变更时，展开用户可见的 `系统 > 快商通模式`，直接选择“选择快商通程序目录”“选择快商通数据目录”，或点击“重新扫描快商通”。明确检测到程序目录或数据目录无效时，每次启动最多自动打开一次对应目录选择框；取消后不会循环弹出。选择器只接受目录；保存后异步重扫，不阻塞界面。旧客户端的数据根目录为 `KuaiShangDataNew`，真实业务数据在其 `db` 下；不得复制、迁移或手动选择其中的数据库文件。
