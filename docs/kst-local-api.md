...
自动发现失败或安装位置变更时，展开用户可见的 `系统 > 快商通模式`，直接选择“选择快商通程序目录”“选择快商通数据目录”，或点击“重新扫描快商通”。重新扫描入口在人工导出模式下也可用，点击后会切回本地 API 并执行扫描。目录状态只在界面中提示，不会自动弹出并可能被其他窗口遮挡的选择框。选择器只接受目录；人工保存的数据目录会参与 Electron 与旧客户端的后续发现。旧客户端的数据根目录为 `KuaiShangDataNew`，真实业务数据在其 `db` 下；不得复制、迁移或手动选择其中的数据库文件。
